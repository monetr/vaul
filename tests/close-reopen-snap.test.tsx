...
// Bug F: closeDrawer schedules `setTimeout(() => setActiveSnapPoint(snapPoints[0]), 500)` at
// src/index.tsx:565-569 with no cleanup. After a drag-close, even if the drawer never visually
// closes (open prop hardcoded), the stale timer fires and overwrites a later setActiveSnapPoint
...
describe('closeDrawer stale timer', () => {
  ...
  test('user-set active snap is not overwritten by the stale 500ms timer from a prior close', async () => {
    ...
    // Hard drag down at index 1 -> useSnapPoints.onRelease (velocity > 2, dismissible) ->
    // closeDrawer() -> schedules stale timer at +500ms to set snap to snapPoints[0] (which is 0
    // in this fixture).
